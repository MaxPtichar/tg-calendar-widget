...
#[cfg(test)] 
    mod is_date_allowed_tests {
        ...
        let config = CalendarConfig { min_date: Some(min_date),max_date: Some(max_date) };
        ...
        let max_date = NaiveDate::from_ymd_opt(2026, 6, 20).unwrap();
        let config = CalendarConfig { min_date: None,max_date: Some(max_date) };
        ...
        let config = CalendarConfig { min_date: None,max_date: None };
